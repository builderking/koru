...
public struct OnboardingView: View {
    ...
    public var body: some View {
        VStack(spacing: KoruSpacing.section) {
            HStack(spacing: 10) {
                ...
                Text("Welcome to Koru").font(.title2.bold())
                Spacer()
                Text("\(step + 1) of 4").foregroundStyle(.secondary)
            }
            Group {
                if step == 0 { valueDemo }
    ...
    private var valueDemo: some View { VStack(spacing: 14) { Image(systemName: "text.bubble.fill").font(.system(size: 42)).foregroundStyle(KoruColors.accent); Text("Remember a fragment. Find the right writing.").font(.title3); Text("Your Library works immediately—without permissions, an account, or a network connection.").foregroundStyle(.secondary); HStack { Text("pus").font(.system(.body, design: .monospaced)).foregroundStyle(KoruColors.accent); Image(systemName: "arrow.right").foregroundStyle(KoruColors.accent); Label("Push changes and open a pull request", systemImage: "text.quote") }.padding().koruAdaptiveSurface() }.multilineTextAlignment(.center).accessibilityElement(children: .combine) }
    private var modeChoice: some View { VStack(alignment: .leading, spacing: 12) { Text("Choose how Koru appears").font(.headline); Picker("Mode", selection: $mode) { Text("Hotkey-only").tag(0); Text("Full").tag(1) }.pickerStyle(.segmented); GroupBox { VStack(alignment: .leading, spacing: 8) { Label(mode == 0 ? "Open recall with a shortcut" : "Also match your tags as you type", systemImage: "keyboard"); Text(mode == 0 ? "No Input Monitoring needed. You can enable more later." : "Full mode needs Accessibility and Input Monitoring.").foregroundStyle(.secondary) }.frame(maxWidth: .infinity, alignment: .leading) } }.frame(maxWidth: 460) }
    private var permissionChoice: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if mode == 1 {
                PermissionRow(
                    title: "Accessibility",
                    detail: "Lets Koru find the active text field and insert after you choose a result.",
    ...
    private var finish: some View { VStack(spacing: 14) { Image(systemName: "checkmark.circle.fill").font(.system(size: 40)).foregroundStyle(KoruColors.accent); Text("Koru is ready").font(.title3); Text("Declined permissions never block the Library. Use the recall hotkey now and revisit Settings whenever you want.").foregroundStyle(.secondary).multilineTextAlignment(.center); Label("Content stays local on your Mac.", systemImage: "lock.shield").foregroundStyle(KoruColors.accent) } }
}

private struct PermissionRow: View {
    var title: String; var detail: String; var state: PermissionState; var action: () -> Void
    var body: some View { GroupBox { HStack(alignment: .top) { VStack(alignment: .leading, spacing: 4) { HStack { Text(title).bold(); Text(state.rawValue.capitalized).font(.caption).foregroundStyle(state == .granted ? KoruColors.accent : KoruColors.muted).padding(.horizontal, 6).padding(.vertical, 2).background(state == .granted ? KoruColors.accentSoft : KoruColors.panel).clipShape(Capsule()) }; Text(detail).font(.callout).foregroundStyle(.secondary) }; Spacer(); Button("Review & Enable", action: action) }.frame(maxWidth: .infinity) } }
}
...
public struct SavedItemEditor: View {
    ...
    public init(item: SavedItem = SavedItem(title: "", behavior: .savedText, plainContent: ""), duplicateWarning: Bool = false) {
        _draft = State(initialValue: item)
        _tagsText = State(initialValue: item.triggerTags.joined(separator: ", "))
        self.duplicateWarning = duplicateWarning
    }
    public var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Saved Text").font(.title2)
            if duplicateWarning {
                Label("Similar content already exists. Review before saving another copy.", systemImage: "exclamationmark.triangle")
                    .foregroundStyle(.orange)
            }
            Text("Content").font(.headline)
            TextEditor(text: $draft.plainContent)
                .font(.body)
                .frame(minHeight: 180)
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(.separator))
            Text("Tags").font(.headline)
            TextField("Comma-separated words or phrases", text: $tagsText)
            Text("Add one or more tags of \(KoruPolicy.minimumTriggerLength)–\(KoruPolicy.maximumTriggerLength) characters. Koru opens when a complete tag is typed.")
                .font(.caption)
    ...
    private func save() {
        do {
            let tags = try SavedItemValidation.validatedTriggerTags(content: draft.plainContent, triggerTags: split(tagsText))
            draft.tags = tags
            draft.matchTerms = tags.map { MatchTerm(value: $0, isPreferredInitialTerm: true, isExactTrigger: true) }
            draft.title = draft.displayTitle
            draft.behavior = .savedText
...
public struct LibraryView: View {
    ...
    private var filtered: [SavedItem] {
        store.items.filter { item in
            let belongs = collection == .recentlyDeleted
                ? item.deletedAt != nil
                : collection == .archived
                    ? item.deletedAt == nil && item.archivedAt != nil
                    : item.deletedAt == nil && item.archivedAt == nil
            let matches = query.isEmpty
                || item.plainContent.localizedCaseInsensitiveContains(query)
                || item.triggerTags.contains(where: { $0.localizedCaseInsensitiveContains(query) })
            return belongs && matches
        }
        .sorted { ($0.isPinned ? 0 : 1, $0.displayTitle.lowercased()) < ($1.isPinned ? 0 : 1, $1.displayTitle.lowercased()) }
    }
    public var body: some View {
        NavigationSplitView {
            ...
            List(selection: $selection) { ForEach(filtered) { item in itemRow(item) } }
                .searchable(text: $query, prompt: "Search content or tags")
    ...
    private func itemRow(_ item: SavedItem) -> some View {
        HStack { Image(systemName: "text.quote").foregroundStyle(KoruColors.accent); VStack(alignment: .leading) { Text(item.displayTitle).lineLimit(1); Text(item.triggerTags.joined(separator: " · ")).font(.caption).foregroundStyle(.secondary).lineLimit(1) }; Spacer(); if item.isPinned { Image(systemName: "pin.fill").foregroundStyle(KoruColors.accent).accessibilityLabel("Pinned") } }
            .tag(item.id).contextMenu { itemMenu(item) }.accessibilityElement(children: .combine)
            .accessibilityLabel("\(item.displayTitle), tags \(item.triggerTags.joined(separator: ", "))\(item.isPinned ? ", pinned" : "")")
    }
    ...
    @ViewBuilder private func detail(_ item: SavedItem) -> some View { ScrollView { VStack(alignment: .leading, spacing: 16) { HStack { Spacer(); Button("Edit") { editor = item }.keyboardShortcut(.return, modifiers: [.command]) }; if !item.triggerTags.isEmpty { HStack { ForEach(item.triggerTags, id: \.self) { Text($0).font(.caption).foregroundStyle(KoruColors.accent).padding(.horizontal, 7).padding(.vertical, 3).background(KoruColors.accentSoft).clipShape(Capsule()) } } }; Text(item.plainContent).textSelection(.enabled).frame(maxWidth: .infinity, alignment: .leading).padding().background(KoruColors.canvasRaised).overlay(RoundedRectangle(cornerRadius: 8).stroke(KoruColors.hairline)).clipShape(RoundedRectangle(cornerRadius: 8)) }.padding(24) } }
    @ViewBuilder private func itemMenu(_ item: SavedItem) -> some View { Button("Edit") { editor = item }; Button("Duplicate") { var copy = item; copy.id = .init(); copy.createdAt = .now; copy.updatedAt = .now; editor = copy }; Button(item.isPinned ? "Unpin" : "Pin") { var copy = item; copy.isPinned.toggle(); try? store.save(copy) }; Divider(); if collection == .active { Button("Archive") { store.move(item.id, to: .archived) } }; if collection != .active { Button("Restore") { store.move(item.id, to: .active) } }; if collection != .recentlyDeleted { Button("Move to Recently Deleted", role: .destructive) { store.move(item.id, to: .recentlyDeleted) } } else { Button("Delete Permanently", role: .destructive) { confirmDelete = item } } }
...
public struct SettingsView: View {
    ...
    @State private var newClipboard = ""
    @State private var confirmReset = false
    public init(clipboard: ClipboardSettingsModel = .init()) { self.clipboard = clipboard }
    public var body: some View {
        TabView {
            Form {
                Toggle("Typed Matching", isOn: $draft.typedMatchingEnabled)
                Text("Matches saved tags as you type in text fields.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Toggle("Selection save icon", isOn: $draft.selectionIconEnabled)
                Toggle("Launch at Login", isOn: $draft.launchAtLogin)
                Toggle("Pause Koru", isOn: $draft.isPaused)
                shortcuts
            }
            .tabItem { Label("General", systemImage: "gear") }

            Form {
                Toggle("Clipboard History", isOn: $draft.clipboardHistoryEnabled)
                LabeledContent("Pasteboard access", value: clipboard.accessDescription)
                Stepper("Retention: \(draft.retentionDays) days", value: $draft.retentionDays, in: 1...30)
                Stepper("Maximum events: \(draft.maximumEvents)", value: $draft.maximumEvents, in: 50...500, step: 50)
                LabeledContent("Retained", value: "\(clipboard.retainedCount) events")
                LabeledContent("Encrypted storage", value: ByteCountFormatter.string(fromByteCount: Int64(clipboard.encryptedBytes), countStyle: .file))
                Stepper("Asset limit: \(draft.maximumAssetMegabytes) MB", value: $draft.maximumAssetMegabytes, in: 32...1024, step: 32)
                exclusionEditor("Never Save Clipboard From", value: $newClipboard, items: $draft.neverSaveClipboardFrom)
            }
            .tabItem { Label("Privacy", systemImage: "hand.raised") }

            Form {
                ForEach(KoruPermission.allCases, id: \.self) { permission in
                    HStack {
                        Text(permission.rawValue.capitalized)
                        Spacer()
                        Text(permissionState(permission).rawValue.capitalized)
                        Button("Review") { store.request(permission) }
                    }
                }
                Button("Refresh Permission State") { store.refreshPermissions() }
                Divider()
                Button("Clear Clipboard History") {
                    clipboard.onClear?()
                    Task { _ = await store.perform(.clearClipboardHistory) }
                }
                Button("Reset Vault…", role: .destructive) { confirmReset = true }
            }
        ...
        .onChange(of: draft) { value in
            store.applySettings(value)
            clipboard.onEnableChanged?(value.clipboardHistoryEnabled)
            clipboard.onRetentionChanged?(value.retentionDays, value.maximumEvents)
            clipboard.onExclusionsChanged?(Set(value.neverSaveClipboardFrom))
        }
        .alert("Reset the entire vault?", isPresented: $confirmReset) {
            Button("Cancel", role: .cancel) {}
            Button("Reset Vault", role: .destructive) { Task { _ = await store.perform(.resetVault) } }
        } message: {
            Text("All saved items and clipboard history will be permanently deleted.")
        }
    }
...
public struct DiagnosticsView: View {
    ...
    private func health(_ label: String, _ value: ServiceHealth) -> some View { HStack { Text(label); Spacer(); Image(systemName: value == .healthy ? "checkmark.circle.fill" : value == .degraded ? "exclamationmark.triangle.fill" : "pause.circle").foregroundStyle(value == .healthy ? KoruColors.accent : value == .degraded ? Color.orange : KoruColors.muted); Text(value.rawValue.capitalized) }.accessibilityElement(children: .combine) }
