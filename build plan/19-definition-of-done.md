...
- [ ] Fresh-input activation is implemented exactly: an initially empty editable field, caret at the beginning, a qualifying typed fragment, no popup from empty focus alone, and no mid-writing activation.
- [ ] `clp` is the locked V1 Clipboard command: it works only from a verified fresh-empty start, leaves the typed characters untouched until explicit result selection, and has a manual Clipboard fallback.
...
- [ ] The Saved text, Quick replacement, and Template behaviors share one canonical saved-item model, while the separate temporary Clipboard scope behaves as specified.
- [ ] Fuzzy fragment retrieval, explicit insertion, and query learning pass deterministic tests.
...
- [ ] Template variables can be filled without leaving the target writing flow.
...
- [ ] Raw keystrokes are held only in the minimum in-memory trigger buffer and are never persisted.
- [ ] Secure text fields, login/authorization surfaces, and configured excluded apps are ignored.
...
- [ ] Clear history, delete all data, pause, exclusions, retention, and export controls work offline.
...
- [ ] Documentation covers installation, permissions, first save, recall, clipboard, exclusions, backup, update, uninstall, and troubleshooting.
